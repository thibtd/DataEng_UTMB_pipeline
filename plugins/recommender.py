...
class Recommender:
    ...
    def preprocess(self):
        ...
        cleaned_data = pd.concat(
            [data, pd.DataFrame(X_scaled, columns=numeric_columns)], axis=1
        )
        # remove variables
    ...
    def inspect_kmeans(self):
        ...
        silhouette_visualizer = SilhouetteVisualizer(
            self.kmeans, colors="yellowbrick", ax=ax2
        )
        ...
        ax4.scatter(
            self.kmeans.cluster_centers_[:, -4],
            self.kmeans.cluster_centers_[:, -5],
            c="red",
            s=200,
            alpha=0.7,
        )
        legend_elements = [
            Line2D(
                ...
                markerfacecolor=plt.cm.viridis(i / (len(cluster_names) - 1)),
                markersize=10,
            )
        ...
        for i, component in enumerate(abs(self.loadings)):
            ranked_features = pd.Series(
                component, index=self.cleaned_data.columns
    ...
    def process_input(self, preferences):
        ...
        distance_list = [
            round_to_nearest_5(i) for i in range(min_dist, max_dist + 1, 5)
        ]
        ...
        flattened_prefs.update(
            {
                key: value
                for key, value in preferences.items()
                if key != "country" and not isinstance(value, list)
            }
        )
        ...
        user_pref = pd.DataFrame(
            data=flattened_prefs, columns=self.cleaned_data.columns, index=[0]
        )
        ...
        user_pref = pd.concat(
            [user_pref, pd.DataFrame(X_scaled, columns=numeric_columns)], axis=1
        )
    ...
    def recommend(
        ...
        def recommend(
            ...
            """
            Recommend events based on user input.
            Parameters:
            ...
            weight (bool, optional): A boolean flag to indicate if weighting should be applied. Default is False.
            to_be_inc (str, optional): A string parameter for additional inclusion criteria. Default is None.
            Returns:
            pd.DataFrame: A DataFrame containing the top N recommended events.
            """
    ...
    def explain_recommendations(self):
        ...
        correlation_with_similarity = self.recommended.corr()["similarity"].sort_values(
            ascending=False
        )
